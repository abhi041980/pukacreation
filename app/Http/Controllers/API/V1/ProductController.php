      ...
      try{
          ...
          if($category_products) {
            foreach ($category_products as $key => $product) {
              ...
              $products['offer_price'] = $pro->offer_price;
              $products['regular_price'] = $pro->regular_price;
    ...
    public function productDetail(Request $request){
      try{    
           ...
           $user = User::where('api_key', $request->api_key)->select('id')->first();
            $data = [];
            $url = route('home'); 
            $product = Product::where('id', $request->product_id)->select('id', 'name', 'url', 'sku', 'quantity', 'featured_image', 'offer_price', 'regular_price', 'description', 'product_description', 'category_id', 'srp')->first(); 
            $data['id'] = $product->id;
            ...
            $data['case_deal'] = CaseDeal::where('status', 1)->where('product_id', $request->product_id)->select('quantity as min_quantity', 'discount', 'max_quantity')->get();
            // $data['gallery'] = ProductImage::where('product_id', $product->product_lot_id)->select('product_image')->get();
