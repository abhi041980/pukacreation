...
if(!in_array($excelFile->getClientOriginalExtension(), ['xls','xlsx'])){
   ...
   $csv = $request->file('file');
    ...
    ->with('error', ''.$cat_no.' is not a valid category'); 
           ...
           return redirect()->route('product.index')
    ->with('error', 'Kindly add a category for '.$row['3'].'.');
       }
       ...
       if(isset($category[4])){
           ...
           $cat_5 = 0;
              ...
              if($row['2']!='' || $row['3']!=''){
                ...
                $case_qty = $row['13'];
                $case_discount = $row['14'];
                
                $user_id=\Auth::user()->id;
                
                
                $product = Product::where('name', $row['3'])->select('id', 'name')->first();
              //  dd($product);
                if($product){
                  ...
                  if($check_sku){
                    return redirect()->route('product.index')
    ->with('error', ''.$check_sku->name.' already have same '.$sku.' SKU');  
                  }
