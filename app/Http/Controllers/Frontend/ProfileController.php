    ...
    if($request->address){
     ...
     if($user_address == null){
      ...
      $cart_products = \DB::table('products')
        ...
        ->select('products.name','products.thumbnail', 'products.offer_price', 'carts.quantity', 'products.regular_price', 'carts.id as c_id', 'products.id', 'products.url', 'carts.id as c_id')
        ...
        $cart_products = \DB::table('products')
        ->join('carts', 'carts.product_id', '=','products.id')
        ->select('products.name','products.thumbnail', 'products.offer_price', 'carts.quantity', 'products.regular_price', 'carts.id as c_id', 'products.id', 'products.url')
        ->where('products.status', 1)
...
if(isset($cart_products)) {
  foreach($cart_products as $cart_product){
    if(isset($cart_product->sale_price)){
      ...
      $sale_price_p = get_zone_price($my_zone, $cart_product->lot_id);
            if($sale_price_p){
             $s_price = $sale_price_p;
           } else{
              $s_price = $cart_product->sale_price;
      }
...
$total_pay_amout = (int)$sale_price-$discount;
...
try {
  ...
  public function userAddress() {
    try{
      ...
      return view('frontend.pages.user_address', compact('user_addresses', 'df_address', 'user_addresses_count', 'cart', 'countries'));
