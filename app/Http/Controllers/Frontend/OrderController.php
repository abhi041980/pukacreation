...
class OrderController extends  Controller{
    ...
    public function  index()
    {
      try {
          ...
          ->select('orders.id','orders.order_nr', 'orders.total_price', 'order_statuses.type', 'order_statuses.id as type_id', 'order_addresses.name', 'orders.current_status', 'orders.created_at')
    ...
    public function  orderDetail($id = null)
    {
      try {
        ...
        $order = \DB::table('orders')
          ->join('order_statuses', 'order_statuses.id', '=', 'orders.current_status')
          ->join('order_addresses', 'order_addresses.id', '=', 'orders.shipping_id')
          ->select('orders.id','orders.order_nr', 'orders.total_price', 'order_statuses.type', 'order_addresses.name', 'order_addresses.mobile', 'order_addresses.address', 'order_addresses.state', 'order_addresses.city', 'order_addresses.pincode', 'orders.discount', 'orders.created_at', 'orders.current_status', 'orders.transaction_id')
          ->where('orders.user_id', $user_id)
          ->where('orders.id', $id)
          ->first();
