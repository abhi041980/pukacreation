...
try {
   ...
   // dd($case_deal);
    ...
    $InstructionVideo = InstructionVideo::where('status', 1)->where('product_id', $product->id)->select('iframe_code', 'name')->first();
    $Categorys = Category::where('status', 1)->select('name', 'id', 'url')->where('id', '!=', 13)->Orderby('name')->get();
    $new_products = Product::where('status', 1)->where('category_id', $product->cat_id)->select('id', 'name', 'url', 'quantity', 'offer_price', 'regular_price', 'thumbnail')->limit(20)->orderby('id', 'desc')->get();
    
      return view('frontend.pages.product-detail', compact('product', 'new_products', 'gallery_imgs', 'similars', 'Categorys', 'reviews', 'one_per', 'two_per', 'three_per', 'four_per', 'five_per', 'all_rating', 'your_rating', 'keyword', 'case_deal', 'case_deal_price', 'InstructionVideo'));
    } else {
      return back();
     }
    }
